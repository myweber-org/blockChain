...
func main() {
	if len(os.Args) < 4 {
		...
		fmt.Println("Key must be set via ENCRYPTION_KEY environment variable (32 bytes for AES-256)")
		os.Exit(1)
	}

	key := []byte(os.Getenv("ENCRYPTION_KEY"))
	if len(key) != 32 {
		fmt.Println("Error: ENCRYPTION_KEY must be exactly 32 bytes for AES-256")
		os.Exit(1)
	}
