...
func (sm *SessionManager) CreateSession(userID int, initialData map[string]interface{}) string {
    sm.mu.Lock()
    defer sm.mu.Unlock()
...
func (sm *SessionManager) UpdateSessionData(sessionID string, key string, value interface{}) bool {
    sm.mu.Lock()
    defer sm.mu.Unlock()
