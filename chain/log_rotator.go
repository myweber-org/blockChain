...
func main() {
	...
	for i := 0; i < 1000; i++ {
		msg := fmt.Sprintf("[%s] Log entry %d: Sample log data for rotation testing\n",
			time.Now().Format(time.RFC3339), i)
		logger.Write([]byte(msg))
		time.Sleep(10 * time.Millisecond)
	}
...
func NewLogRotator(path string) (*LogRotator, error) {
	rotator := &LogRotator{basePath: path}
	if err := rotator.openCurrentFile(); err != nil {
		return nil, err
	}
	return rotator, nil
...
func (lr *LogRotator) rotate() error {
	if lr.currentFile != nil {
		lr.currentFile.Close()
		timestamp := time.Now().Format("20060102_150405")
		oldPath := lr.basePath + "." + timestamp
		if err := os.Rename(lr.basePath, oldPath); err != nil {
			return err
		}
