...
func (rl *RotatingLogger) cleanOldFiles() {
	...
	matches, err := filepath.Glob(pattern)
	if err != nil {
		log.Printf("Failed to list archive files: %v", err)
		return
	}
	...
	for i := 0; i < len(matches)-rl.maxFiles; i++ {
		if err := os.Remove(matches[i]); err != nil {
			log.Printf("Failed to remove old file %s: %v", matches[i], err)
		}
	}
}
...
func main() {
	logger, err := NewRotatingLogger("app.log", 10) // 10 MB max size
	if err != nil {
