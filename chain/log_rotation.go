...
func (rl *RotatingLogger) rotate() error {
	...
	oldName := fmt.Sprintf("%s.log", rl.baseName)
	newName := fmt.Sprintf("%s.%d.log.gz", rl.baseName, time.Now().Unix())
...
func (rl *RotatingLogger) openNewFile() error {
    ...
    filename := filepath.Join(logDir, fmt.Sprintf("%s.%d.log", rl.baseName, rl.sequence))
    file, err := os.OpenFile(filename, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644)
    if err != nil {
        return err
    }
...
func NewRotatingLogger(baseName string) (*RotatingLogger, error) {
    ...
    rl.writer = io.MultiWriter(os.Stdout, rl.currentFile)
    return rl, nil
}
...
func (rl *RotatingLogger) openNextFile() error {
    ...
    filename := filepath.Join(logDirectory, fmt.Sprintf("%s_%d.log", rl.baseName, rl.fileIndex))
    file, err := os.OpenFile(filename, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644)
    if err != nil {
        return err
    }
...
func (rl *RotatingLogger) cleanupOldFiles() {
    ...
    for _, match := range matches {
        ...
        numStr := strings.TrimPrefix(strings.TrimSuffix(base, ".log"), rl.baseName+"_")
