...
func (rl *RotatingLogger) rotate() error {
	...
	rl.rotationCount++
	archiveName := fmt.Sprintf("%s.%d.%s.gz", rl.basePath, rl.rotationCount, time.Now().Format("20060102150405"))
...
func (rl *RotatingLogger) cleanupOld() {
	compressedFiles, err := filepath.Glob(filepath.Join(logDir, rl.baseName+"_*.log.gz"))
	if err != nil {
		return
	}
