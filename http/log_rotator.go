...
func NewLogRotator(filePath string, maxSize int64, maxFiles int) (*LogRotator, error) {
	rotator := &LogRotator{
		filePath: filePath,
		maxSize:  maxSize,
		maxFiles: maxFiles,
...
func (lr *LogRotator) cleanOldFiles() {
	...
	if len(matches) > lr.maxFiles {
		filesToRemove := len(matches) - lr.maxFiles
...
func NewLogRotator(filePath string, maxSize int64, maxFiles int, rotationTime time.Duration) (*LogRotator, error) {
    lr := &LogRotator{
        FilePath:    filePath,
        MaxSize:     maxSize,
        MaxFiles:    maxFiles,
    ...
    if rotationTime > 0 {
        lr.ticker = time.NewTicker(rotationTime)
        go lr.timeBasedRotation()
    }
...
func (lr *LogRotator) Write(p []byte) (int, error) {
    if lr.currentSize+int64(len(p)) > lr.MaxSize && lr.MaxSize > 0 {
        if err := lr.rotate(); err != nil {
            return 0, err
        }
    }
...
func (lr *LogRotator) timeBasedRotation() {
    for {
        select {
        case <-lr.ticker.C:
            if err := lr.rotate(); err != nil {
