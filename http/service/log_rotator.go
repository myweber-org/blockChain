...
func NewRotatingLogger(basePath string, maxSizeMB int, maxFiles int, compress bool) (*RotatingLogger, error) {
	rl := &RotatingLogger{
		basePath:    basePath,
...
func (rl *RotatingLogger) rotate() error {
	...
	if rl.compressOld && rl.fileCounter > 1 {
		go rl.compressFile(oldFilename)
	}
...
func (rl *RotatingLogger) compressFile(filename string) {
	if !strings.HasSuffix(filename, ".log") {
		return
	}

	compressedName := filename + ".gz"
	log.Printf("Compressing %s to %s", filename, compressedName)
...
func main() {
	logger, err := NewRotatingLogger("/var/log/myapp/app", 10, 5, true)
	if err != nil {
		log.Fatal(err)
	}
	defer logger.Close()
...
func (lr *LogRotator) cleanupOldBackups() error {
    ...
    for _, file := range files {
        ...
        timestamps = append(timestamps, modTime)
        fileMap[modTime] = file
    }
...
func (rl *RotatingLogger) rotate() error {
	...
	backupPath := filepath.Join(logDir, fmt.Sprintf("%s-%s.log.gz",
		filepath.Base(rl.filePath[:len(rl.filePath)-4]), timestamp))
...
func (rl *RotatingLogger) cleanupOldBackups() error {
	pattern := filepath.Join(logDir, filepath.Base(rl.filePath[:len(rl.filePath)-4])+"-*.log.gz")
	matches, err := filepath.Glob(pattern)
	if err != nil {
		return err
	}
