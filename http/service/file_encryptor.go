...
func main() {
	...
	case "encrypt":
		if len(os.Args) != 5 {
			fmt.Println("Usage: file_encryptor encrypt <input> <output> <key_file>")
			os.Exit(1)
		}
		key, err := os.ReadFile(os.Args[4])
		if err != nil {
			fmt.Printf("Read key failed: %v\n", err)
			os.Exit(1)
		}
	...
	case "decrypt":
		if len(os.Args) != 5 {
			fmt.Println("Usage: file_encryptor decrypt <input> <output> <key_file>")
			os.Exit(1)
		}
		key, err := os.ReadFile(os.Args[4])
		if err != nil {
	...
	case "genkey":
		if len(os.Args) != 3 {
			fmt.Println("Usage: file_encryptor genkey <output_key_file>")
			os.Exit(1)
		}
		key, err := generateKey()
		if err != nil {
...
func main() {
	...
	case "decrypt":
		...
		var keyHex string
		fmt.Scanln(&keyHex)
		key, err := hexStringToBytes(keyHex)
		if err != nil {
...
func hexByte(a, b byte) (byte, error) {
	toValue := func(c byte) (byte, error) {
		switch {
		case c >= '0' && c <= '9':
			return c - '0', nil
		case c >= 'a' && c <= 'f':
