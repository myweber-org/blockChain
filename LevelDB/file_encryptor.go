...
func main() {
    ...
    case "encrypt":
        if len(os.Args) < 5 {
            fmt.Println("Encrypt requires key argument")
            fmt.Println("Usage: go run file_encryptor.go encrypt <input> <output> <hex_key>")
            os.Exit(1)
        }
    ...
    case "decrypt":
        if len(os.Args) < 5 {
            fmt.Println("Decrypt requires key argument")
            fmt.Println("Usage: go run file_encryptor.go decrypt <input> <output> <hex_key>")
            os.Exit(1)
        }
...
func main() {
	...
	encrypted, err := encryptData([]byte(message), key)
	if err != nil {
